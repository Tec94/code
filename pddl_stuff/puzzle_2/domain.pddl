(define (domain matrix)
    ...
    (:predicates
        ...
        (numbr_at_pos ?n - value ?x ?y - position)
        (empty_at_pos ?x ?y - position)
    )
    (:action move_left
        ...
        :precondition (and
            ...
            (not(left_most ?xf))
            (empty_at_pos ?xt ?yf)
            (numbr_at_pos ?num ?xf ?yf)
        )
        :effect (and
            (not(numbr_at_pos ?num ?xf ?yf))
            (empty_at_pos ?xf ?yf) 
            (numbr_at_pos ?num ?xt ?yf)
            (not(empty_at_pos ?xt ?yf))
        )
    )
    (:action move_right
        :parameters (?num - value ?xf ?yf ?xt - position)
    ...
    (:action move_up
        ...
        :effect (and
            (not(numbr_at_pos ?num ?xf ?yf))
            (empty_at_pos ?xf ?yf)
            (numbr_at_pos ?num ?xf ?yt)
            (not(empty_at_pos ?xf ?yt)) 
        )
    )
    (:action move_down
        :parameters (?num - value ?xf ?yf ?yt - position)
